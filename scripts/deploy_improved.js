// Script para desplegar el contrato VotingSystem_Complete a la blockchain
const { ethers } = require("hardhat");
const fs = require("fs");
const path = require("path");

async function main() {
  console.log("Iniciando despliegue del contrato VotingSystem_Complete...");

  // Obtener la factory del contrato
  const VotingSystem = await ethers.getContractFactory("VotingSystem_Complete");
  
  // Desplegar el contrato
  console.log("Desplegando VotingSystem_Complete...");
  const votingSystem = await VotingSystem.deploy();

  // Esperar a que finalice el despliegue
  await votingSystem.deployed();
  
  console.log("Contrato VotingSystem_Complete desplegado en:", votingSystem.address);
  
  // Guardar la dirección del contrato en un archivo para fácil acceso
  const deploymentInfo = {
    contractAddress: votingSystem.address,
    contractName: "VotingSystem_Complete",
